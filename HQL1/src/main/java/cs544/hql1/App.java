...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Flight> flights = session.createQuery("from Flight f  where f.airplane.capacity > 500 and f.origin.country='USA' and" +
                " f.destination !='USA' ").list();
        ...
        List<Airline> airlines = session.createQuery("select distinct a from Airline a join a.flights as f " +
                "join f.airplane as airp where airp.model = 'A380' ").list();
        ...
        flights = session.createQuery("select f from Flight f join f.airline air " +
                "join f.airplane airp  where airp.model='747' and not air.name ='Star Alliance'  ").list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
            System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
        ...
        Query query = session.createQuery("select f from Flight f " +
                "where f.departureTime < '12:00:00' and f.departureDate ='2009-08-07' ");
        flights = query.list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
